...
webhookcnl_id = '878455078527303761' #받아온 웹훅 보내주는 채널ID
my_server = '[𝕐𝕄]파이브엠커뮤니티' #자기서버 배너이름
...
async def on_connect():
    ...
    game = discord.Game('야미야 배너 [배너이름]')
    await client.change_presence(status=discord.Status.online, activity=game)


@client.event
async def on_message(message):
    if message.content.startswith('야미야 배너'):
